...
async function isReplay(ciphertextB64: string): Promise<boolean> {
  const hashBuf = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(ciphertextB64));
...
async function advanceChainKey(
  ...
  const messageKey = await hkdf(chainKey, new Uint8Array([0x01]), info, 32);
  // nextChainKey = HKDF(chainKey, salt=0x02, info)
  const nextChainKey = await hkdf(chainKey, new Uint8Array([0x02]), info, 32);
 ...
 */
export async function resolveUsername(username: string): Promise<{ id: string; username: string }> {
 ...
 */
export async function resolveUserId(userId: string): Promise<{ id: string; username: string }> {
...
export async function sendFirstMessage(
  ...
  const otpPub = bundle.one_time_pre_key ? decodeBase64(bundle.one_time_pre_key.public_key) : null;
...
export async function sendMessage(
  ...
  const { messageKey, nextChainKey } = await advanceChainKey(currentChainKey, CHAIN_RATCHET_INFO);
...
export async function processIncomingMessage(
  ...
  if (type === 'pre_key_signal_message') {
    ...
    const identity = await getIdentityKeyPair(encryptionKey);
    const signedPreKey = await getSignedPreKey(envelope.signedPreKeyId, encryptionKey);
    ...
    if (!session) {
      throw new Error(`No session found for sender ${senderId}. Cannot decrypt signal_message.`);
    }
    ...
    const { messageKey, nextChainKey } = await advanceChainKey(chainKey, CHAIN_RATCHET_INFO);
  ...
  if (payload.recipientId && payload.recipientId !== myId) {
    throw new Error('Message was not intended for this user (recipient mismatch).');
  }
  ...
  if (payload.timestamp && Math.abs(Date.now() - payload.timestamp) > MAX_MESSAGE_AGE_MS) {
    console.warn(
      `[Chat] Message from ${senderId} has stale timestamp (${new Date(payload.timestamp).toISOString()}). Allowing for offline delivery.`
    );
  }
...
export async function loadMessages(
  ...
  const rows = await db.messages.where('contactId').equals(contactId).sortBy('timestamp');
